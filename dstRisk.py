...
def doBayesianTensor(incidencesDict, days, correction):
    ...
    weekTrace = relativeRiskTraceBayesian(xbefore_tensor.sum(axis=1),
                                          np.around(
                                              nbefore_tensor.sum(axis=1) / days),
                                          x0_tensor.sum(axis=1),
                                          np.around(
                                              n0_tensor.sum(axis=1) / days),
                                          xafter_tensor.sum(axis=1),
                                          np.around(
                                              nafter_tensor.sum(axis=1) / days),
                                          correction=correction,
                                          days=days)
...
def sampleTraces(day_of_interest, day_of_correction,
                 diseasesDict=None,
                 totalDict=None,
                 correction=None,
                 days=7,
    ...
    for studyGrp_index, studyGrp in enumerate(all_groups):
        ...
        for i, di in enumerate(list(day_of_interest)):
            ...
            incidences_dst_0 += np.array([observations[(d + 1)] for d in index_0])
            incidences_dst_before += np.array([observations[(d + 1)] for d in index_before])
            incidences_dst_after += np.array([observations[(d + 1)] for d in index_after])
...
if __name__ == '__main__':
    ...
    strtDay = np.array([96, 460, 824, 1188, 1531, 1895,
                        2259, 2630, 2994, 3358, 3722, 4086, 4450])
    endDay = np.array([299, 670, 1034, 1398, 1769, 2133,
                       2497, 2868, 3232, 3596, 3960, 4324, 4688])
    easterDay = np.array([110, 467, 817, 1202, 1559, 1909,
                          2294, 2651, 3036, 3386, 3743, 4128, 4478])
    tksGvgDay = np.array([331, 695, 1059, 1423, 1787, 2158,
                          2522, 2886, 3250, 3614, 3985, 4349, 4713])

    with open('INP_COND_ENROLL_SUMMARY.bpickle3', 'rb') as f:
        summDict = pickle.load(f)
        COND_DICT = summDict['COND_DICT']
        ENROLL_DICT = summDict['ENROLL_DICT']    
    
#    pooledDisDict, pooledTot = poolAllStrats(allDiseaseDict, 
#                                             allTotalEnrollNumDict)
#    
#    pooledBySys = groupDiseasesBySystem(pooledDisDict,  mappingDiseases)

# Bayesian Method (all diseases ...) ------------------------------------------

    incidencesSpr, dayTraceSpr, weekTraceSpr = sampleTraces(day_of_interest=strtDay,
                                                            day_of_correction=easterDay,
    ...
    incidencesAut, dayTraceAut, weekTraceAut = sampleTraces(day_of_interest=endDay,
                                                            day_of_correction=tksGvgDay,
                                                            diseasesDict=COND_DICT,
                                                            totalDict=ENROLL_DICT,
                                                            correction='Autumn',
                                                            days=7,
    ...
    df_dst_day_bothsig_bayes.to_csv('./results/interestingResultsBothSig_Bayesian.csv')
