...
PART = 'AllStatesWithDst'


INPUT = './data/INP_COND_ENROLL_SUMMARY_%s.bpkl3' % PART
OUTPATH = './results_%s' % PART + 'PseudoDst'
...
MEMORIAL = [datetime.date(2003, 5, 26),
...
PSEUDO_DST_SPRING_DAY = np.array([(d - START_DATE).days for d in DST_SPRING]) + 28
PSEUDO_DST_AUTUMN_DAY = np.array([(d - START_DATE).days for d in DST_AUTUMN]) - 28
...
MEMORIAL_DAY = np.array([(d - START_DATE).days for d in MEMORIAL])
LABOR_DAY = np.array([(d - START_DATE).days for d in LABOR])

SPRING_AVOID = np.concatenate((PRESIDENT_DAY, WST_EASTER_DAY, ST_PATRICKS_DAY, MEMORIAL_DAY))
AUTUMN_AVOID = np.concatenate((TKS_GIVING_DAY, VETERANS_DAY, COLUMBUS_DAY, LABOR_DAY))
...
PSEUDO_DST_SPRING_CONTROL = []
for dst in PSEUDO_DST_SPRING_DAY:
    
    if dstOverlapHoliday(dst, SPRING_AVOID, 2):
        
        PSEUDO_DST_SPRING_CONTROL.append(2)
        
    else:
        
        if dstOverlapHoliday(dst, SPRING_AVOID, 3):
            
            PSEUDO_DST_SPRING_CONTROL.append(3)
            
        else:
            
            if dstOverlapHoliday(dst, SPRING_AVOID, 1):
            
                PSEUDO_DST_SPRING_CONTROL.append(1)
...
PSEUDO_DST_AUTUMN_CONTROL = []
for dst in PSEUDO_DST_AUTUMN_DAY:
    
    if dstOverlapHoliday(dst, AUTUMN_AVOID, 2):
        
        PSEUDO_DST_AUTUMN_CONTROL.append(2)
        ...
        if dstOverlapHoliday(dst, AUTUMN_AVOID, 1):
            ...
            PSEUDO_DST_AUTUMN_CONTROL.append(1)
            
        else:
            
            if dstOverlapHoliday(dst, AUTUMN_AVOID, 3):
                
                PSEUDO_DST_AUTUMN_CONTROL.append(3)
...
print('Expectation is calculated by ±', PSEUDO_DST_SPRING_CONTROL,
      'weeks in Spring')

print('and ±', PSEUDO_DST_AUTUMN_CONTROL, 'weeks in Autumn')
...
# Frequentist's Method (all diseases ...) -------------------------------------
    ...
    columnNames = ['Condition', 'System', 
                   ...
                   ci_name + ' (Spring)', 
                   'Bonferroni Corrected ' + ci_name + ' (Spring)', 
                   'Spring Significant?']
    ...
    df_dst_start_frequ, plotDataDict_start_frequ = sampleTraces(day_of_interest=PSEUDO_DST_SPRING_DAY,
                                                                week_of_control=PSEUDO_DST_SPRING_CONTROL,
    ...
    columnNames = ['Condition', 'System', 
                   ...
                   ci_name + ' (Autumn)', 
                   'Bonferroni Corrected ' + ci_name + ' (Autumn)', 
                   'Autumn Significant?']
    
    df = pd.DataFrame(columns=columnNames)
    plotDataDict = {}
    
    df_dst_end_frequ, plotDataDict_end_frequ = sampleTraces(day_of_interest=PSEUDO_DST_AUTUMN_DAY,
                                                            week_of_control=PSEUDO_DST_AUTUMN_CONTROL,
                                                            diseasesDict=COND_DICT,
                                                            totalDict=ENROLL_DICT,
                                                            correction='Autumn',
                                                            days=7,
...
# Half-Bayesian Method (all diseases ...) -------------------------------------
    ...
    df_dst_start_hfbayes, plotDataDict_start_hfbayes = sampleTraces(day_of_interest=PSEUDO_DST_SPRING_DAY,
                                                                    week_of_control=PSEUDO_DST_SPRING_CONTROL,
                                                                    diseasesDict=COND_DICT,
                                                                    totalDict=ENROLL_DICT,
                                                                    correction='Spring',
    ...
    df_dst_end_hfbayes, plotDataDict_end_hfbayes = sampleTraces(day_of_interest=PSEUDO_DST_AUTUMN_DAY,
                                                                week_of_control=PSEUDO_DST_AUTUMN_CONTROL,
                                                                diseasesDict=COND_DICT,
                                                                totalDict=ENROLL_DICT,
                                                                correction='Autumn',
...
# Bayesian Method (all diseases ...) ------------------------------------------
    ...
    incidencesSpr, dayTraceSpr, weekTraceSpr = sampleTraces(day_of_interest=PSEUDO_DST_SPRING_DAY,
                                                            week_of_control=PSEUDO_DST_SPRING_CONTROL,
                                                            diseasesDict=COND_DICT,
                                                            totalDict=ENROLL_DICT,
                                                            correction='Spring',
    ...
    incidencesAut, dayTraceAut, weekTraceAut = sampleTraces(day_of_interest=PSEUDO_DST_AUTUMN_DAY,
                                                            week_of_control=PSEUDO_DST_AUTUMN_CONTROL,
                                                            diseasesDict=COND_DICT,
                                                            totalDict=ENROLL_DICT,
                                                            correction='Autumn',
